...
trainedModelDiff <- createRNNModel(epochs = 150, X_train, y_train_diff, X_test, y_test_diff)

model_rnn_diff <- trainedModelDiff$model
trainingTimeRNNDiff <- trainedModelDiff$timeTrainRNN


y_pred_diff = model_rnn %>% predict(X_test)


plotModel(model_rnn_diff, lower, upper)
print(trainingTimeRNNDiff)
...
plot2Models <- function(model1, model2, lower, upper){
  x <- seq(from=lower[1],to=upper[1],length.out=100)
  y <- seq(from=lower[2],to=upper[2],length.out=100)
  ...
  z1 <- model1 %>% predict(x_pred)
  z2 <- model2 %>% predict(x_pred)
...
plot2Models(model_rnn, model_rnn_diff, lower, upper)
