...
dfF1 <- generateDataPoints(n = 100, f = f1)
dfF2 <- generateDataPoints(n = 50, f = f1)
...
n <- 15  # figure with 15x15 data points
x_range <- range(input_data[, "x"])
y_range <- range(input_data[, "y"])
z_range <- range(input_data[, "z"])

# Generate grid points for latent space
grid_x <- seq(x_range[1], x_range[2], length.out = n)
grid_y <- seq(y_range[1], y_range[2], length.out = n)
grid_z <- seq(z_range[1], z_range[2], length.out = n)
